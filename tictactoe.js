...
async function gameLoop() {
  while (!win) {
    ...
    move = validate(move);
    if (move) {
      mks = await mark(move, mks, player);
      draw();
      win = isWin(move, mks, player);
...
async function initGame() {
  ...
  console.log(
    chalk.cyanBright(
      `Current Score: X's: ${score[0]} / O's: ${score[1]}`
    ));
...
async function mark(pos, arr, pl) {
  const i = pos - 1;
  if (typeof arr[i] === 'number') {
    arr[pos - 1] = pl === 1 ? xmk : omk;
  } else {
...
module.exports = { mark, isWin, getInput, validate, xmk };
